...
-- 취소 진행 상태 테이블 (tbl_cancel_status)
CREATE TABLE IF NOT EXISTS tbl_cancel_status
(
    cancel_status_id INT NOT NULL AUTO_INCREMENT COMMENT '취소상태고유ID',
    status_name      VARCHAR(50) NULL COMMENT '취소 진행 상태 (예: 취소요청, 취소완료, 취소거부 등)',
    
    CONSTRAINT pk_tbl_cancel_status PRIMARY KEY (cancel_status_id)
) ENGINE=InnoDB COMMENT='취소 진행 상태 테이블';
...
(
    ...
    user_birthdate        DATE         NULL COMMENT '회원 생년월일',
    user_phone            VARCHAR(11)  NULL COMMENT '회원 휴대폰 번호',
    user_nickname         VARCHAR(50) NOT NULL COMMENT '회원닉네임',
    profile_img           VARCHAR(100) NULL COMMENT '프로필 이미지 경로',
    profile_img_ori_name  VARCHAR(100) NULL COMMENT '프로필 이미지 원본 이름',
    profile_img_sys_name  VARCHAR(300) NULL COMMENT '프로필 이미지 시스템 이름',
    session_id            VARCHAR(100) NULL COMMENT '자동로그인 정보',
    user_role             VARCHAR(20) NULL COMMENT '관리자 여부 (ADMIN, USER, ...)',
    create_dt             DATETIME DEFAULT NOW() COMMENT '회원가입일자',
...
(
    ...
    CONSTRAINT pk_tbl_search  PRIMARY KEY (search_id),
...
(
    ...
    user_id          INT NULL COMMENT '회원 아이디',
    road_address     VARCHAR(255) NULL COMMENT '도로명주소',
    jibun_address    VARCHAR(255) NULL COMMENT '지번주소',
    detail_address   VARCHAR(255) NULL COMMENT '상세주소',
    extra_address    VARCHAR(255) NULL COMMENT '추가 주소 정보 (예: 동/층/호 등)',
    postcode         VARCHAR(15) NULL COMMENT '우편번호',
    addr_name        VARCHAR(50) NULL COMMENT '배송지(수취인) 이름',
    receiver_phone   VARCHAR(15) NULL COMMENT '수취인전화번호',
    primary_addr     BOOLEAN NULL COMMENT '대표배송지 여부',
    delivery_request VARCHAR(255) NULL COMMENT '배송요청사항',

    CONSTRAINT pk_tbl_delivery_address PRIMARY KEY (addr_id),
...
(
    ...
    addr_id        INT NULL COMMENT '배송지 고유 ID',
    total_price    INT NULL COMMENT '주문의 총 금액',
    create_dt      DATETIME DEFAULT NOW() COMMENT '주문 생성 날짜',
...
(
    ...
    description      TEXT NULL COMMENT '도서 설명',
...
(
    ...
    secondhand_description   TEXT NULL COMMENT '상품 설명',
    secondhand_price         INT NULL COMMENT '상품 가격',
    secondhand_date          DATETIME NULL COMMENT '상품 발매일',
    create_dt                DATETIME DEFAULT NOW() COMMENT '상품 등록일',
    sales_yn                 CHAR(1) NULL DEFAULT 'Y' COMMENT '판매 가능 여부 (Y/N)',
...
(
    ...
    cart_created_dt DATETIME DEFAULT NOW() COMMENT '장바구니 생성일',
...
CREATE TABLE IF NOT EXISTS tbl_cart_item (
    ...
    cart_item_created_dt DATETIME DEFAULT NOW() COMMENT '장바구니 항목 등록일',
...
(
    ...
    quantity               INT NULL COMMENT '취소신청 상품수량',
    cancel_reason          VARCHAR(255) NULL COMMENT '취소 사유 상세 설명',
    return_reason          VARCHAR(255) NULL COMMENT '거부, 반려에 대한 상세 설명',
    cancel_status_id       INT NULL COMMENT '취소 상태 고유 ID (예: 요청, 승인, 거절, 환불완료)',
    create_dt              DATETIME DEFAULT NOW() COMMENT '취소신청일시',
...
(
    ...
    order_id      INT NULL COMMENT '주문 고유 ID',
    ...
    create_dt     DATETIME DEFAULT NOW() COMMENT '결제 발생 날짜',
...
(
    ...
    user_id          INT NULL COMMENT '문의글 작성 회원 아이디',
    inquiry_title    VARCHAR(100) NULL COMMENT '문의 제목',
    inquiry_content  VARCHAR(800) NULL COMMENT '문의 내용',
    inquiry_image    VARCHAR(300) NULL COMMENT '문의에 포함된 이미지',
    create_dt        DATETIME DEFAULT NOW() COMMENT '문의글 작성일',
    modify_dt        DATETIME NULL COMMENT '문의글 수정일',
    inquiry_reply_yn CHAR(1) NULL DEFAULT 'N' COMMENT '문의글 답변 여부 (Y/N)',

    CONSTRAINT pk_tbl_product_inq PRIMARY KEY (inquiry_id),
...
(
    ...
    user_id                INT NULL COMMENT '문의글 답변 작성자 회원 아이디',
    inquiry_id             INT NOT NULL COMMENT '상품 문의 아이디',
    inquiry_reply_content  VARCHAR(300) NULL COMMENT '상품 문의 답변 내용',
    inquiry_reply_dt       DATETIME DEFAULT NOW() COMMENT '상품 문의 답변 일자',
...
(
    ...
    order_item_id   INT NULL COMMENT '주문 상세 항목 고유 ID',
    review_rating   INT NULL COMMENT '1~5점 평점',
    review_comment  TEXT NULL COMMENT '후기 상세 내용',
    create_dt       DATETIME DEFAULT NOW() COMMENT '후기 작성 날짜',
...
(
    ...
    notice_create_dt  DATETIME DEFAULT NOW() COMMENT '공지사항 작성일자',
...
(
    ...
    qna_create_dt  DATETIME DEFAULT NOW() COMMENT 'Q&A 작성일자',
...
(
    ...
    faq_create_dt  DATETIME DEFAULT NOW() COMMENT 'FAQ 작성일자',
...
(
    ...
    CONSTRAINT pk_tbl_delete_user_log PRIMARY KEY (delete_log_id)
) ENGINE=InnoDB COMMENT='탈퇴 회원 로그 테이블';

-- 2-25. 사용자 로그인 로그 (login_log)
CREATE TABLE IF NOT EXISTS tbl_login_log
(
    login_log_id   INT NOT NULL AUTO_INCREMENT COMMENT '로그인 로그 아이디',
    user_id        INT NOT NULL COMMENT '회원 아이디',
    login_dt       DATETIME DEFAULT NOW() COMMENT '로그인 날짜',
    login_browser  VARCHAR(300) NULL COMMENT '로그인 브라우저 정보',
    ip_addr        VARCHAR(50) NULL COMMENT '로그인 아이피',

    CONSTRAINT pk_tbl_login_log      PRIMARY KEY (login_log_id),
